...
class Map {
	...
	public function generate() {
    	...
    	for($i=0; $i<count($selectImage); $i++){
	    	for($r=0; $r<$this->getRepeatImages(); $r++){
	    		$this->config['map'][] = array("src"=>$filenames[$selectImage[$i]], "found"=>false, "turned"=>false);
	    	}
	    }
	...
	public function tryCombination(Array $turnedImages) {
		...
		$result = true;
		for($i=0; $i<count($turnedImages); $i++) {
			if($this->config['map'][$turnedImages[0]]['src'] !== $this->config['map'][$turnedImages[$i]]['src']){
				$result = false;
			}
		}
